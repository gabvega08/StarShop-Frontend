...
export function NFTCard({ nft }: { nft: NFT }) {
  const rarity = rarityMap[nft.category] || {
    label: 'Common',
  ...
  return (
    <Card className="overflow-hidden bg-white/10 border border-white/20 rounded-2xl shadow-none">
      <div className="relative h-40 bg-white">
        ...
        <span
          className={`absolute top-3 right-3 px-3 py-1 rounded-full text-xs font-medium ${rarity.color}`}
        >
          {rarity.label}
        </span>
      </div>
      ...
      <div className=" p-5">
        ...
        <Badge className="bg-blue-900/60 text-blue-200 text-xs font-medium mb-2">
          Purchase Reward
        </Badge>
        ...
        <div className="flex gap-3">
          <Button
            variant="outline"
            size="sm"
            className="flex-1 bg-[#0E0E1B] border-white/20 text-white hover:bg-white/10"
          >
          ...
          <Button
            variant="outline"
            size="sm"
            className="flex-1 border-white/20 bg-[#0E0E1B] text-white hover:bg-white/10"
          >
