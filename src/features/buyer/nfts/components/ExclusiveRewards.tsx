      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white/5 border border-white/20 rounded-2xl p-6 flex flex-col h-full">
          ...
          <p className="text-white/60 text-sm mb-5">
            Earn exclusive NFTs with every clothing purchase. The more you shop,
            the rarer the NFTs you&apos;ll collect.
          </p>
          <div className="flex gap-4 mb-6">
            {purchaseRewards.map((reward, i) => (
              <div
                key={i}
                className="flex-1 bg-white/10 min-h-48 rounded-xl p-3 flex flex-col"
              >
          ...
          <Button className="w-full bg-purple-600 hover:bg-purple-700 text-white text-base font-semibold py-2 rounded-md mt-auto">
        ...
        <div className="bg-white/5 border border-white/20 rounded-2xl p-6 flex flex-col h-full">
          ...
          <p className="text-white/60 text-sm mb-5">
            Redeem these special NFT coupons for discounts, exclusive access to
          ...
          <div className="flex flex-col gap-3 mb-6">
            {coupons.map((coupon, i) => (
              <div
                key={i}
                className="flex items-center justify-between min-h-24 bg-transparent rounded-xl px-4 py-3 border-2 border-dashed border-white"
              >
                <div>
                ...
                <div className="flex flex-col items-end gap-2">
                  <span
                    className={`text-xs font-semibold ${coupon.statusColor}`}
                  >
                    {coupon.status}
                  </span>
                  <Button
                    variant="ghost"
                    className="text-white/80 px-3 py-1 h-auto text-xs"
                  >
                    {coupon.button}
                  </Button>
                </div>
          ...
          <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white text-base font-semibold py-2 rounded-md mt-auto">
            Redeem Coupon
          </Button>
        </div>
      </div>
