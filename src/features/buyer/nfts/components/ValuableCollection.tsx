  ...
  return (
    <section className="mb-12 border border-white/20 rounded-2xl p-6 bg-gradient-to-br from-[rgba(43,19,70,0.4)] via-[rgba(39,21,71,0.4)] via-[rgba(29,27,73,0.4)] to-[rgba(21, 36, 197, 0.4)]">
      <h2 className="text-2xl font-bold text-white mb-1">
        Valuable NFT Collection
      </h2>
      <p className="text-white/70 mb-6">
        Your most valuable NFTs that can be traded or sold on the Stellar
        marketplace.
      </p>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {valuableNfts.map((nft, i) => (
          <div
            key={i}
            className="bg-white/5 border border-white/20 rounded-2xl overflow-hidden flex flex-col"
          >
            <div className="relative h-32 bg-white">
              ...
              <span
                className={`absolute top-3 right-3 px-3 py-1 rounded-full text-xs font-medium ${nft.badge.color}`}
              >
                {nft.badge.label}
              </span>
            </div>
